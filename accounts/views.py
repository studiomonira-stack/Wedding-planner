...
def dashboard(request):
    ...
    # TVINGA fram planerare-vy så vi kan testa
    profil.roll = 'planerare'
    profil.save()
    
    brollop_lista = Brollop.objects.filter(planerare=request.user)
    
    # Om inga bröllop finns, skapa ett automatiskt
    if not brollop_lista.exists():
        Brollop.objects.create(planerare=request.user, namn="Test Bröllop")
        brollop_lista = Brollop.objects.filter(planerare=request.user)
